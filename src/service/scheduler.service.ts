...
import DbConnection from "../DatabaseAccessLayer/dbConnection";
import { Job_Details, Template_IDs } from "../Shared/constants";
import {
  DecryptData,
  EncryptData,
  GenerateRandomBytes,
...
export default class SchedulerService extends DbConnection {
  ...
  SendAppoinmentReminder = async (req: Request, res: Response) => {
    ...
    try {
        ...
        await appoinments.map(async (item) => {
          ...
          try {
            const result = await axios.post(
              ...
              { templateId: Template_IDs.Reminder, email: Email }
            );
            ...
            await axios.post(
              (process.env.EMAIL_ENGINE_URL as string) + "/users/sendmail",
              {
                templateId: Template_IDs.Alert,
                email: process.env.ADMIN_MAIL_ID,
      ...
      if (jobObject.Run_Status == "R") jobObject.Run_Status = "C";
      await this.schedulerDA.CreateOrUpdateJob(jobObject);
      ...
      for (let i = 0; i < refresh_token_list.length; i++) {
        ...
        try {
          ...
          const response = await axios.post(
            `https://oauth2.googleapis.com/token?client_id=${process.env.GOOGLE_CLIENT_ID}&client_secret=${process.env.GOOGLE_CLIENT_SECRET}&refresh_token=${decRefreshToken}&grant_type=refresh_token`,
            {
              headers: {
                "Content-Type":
