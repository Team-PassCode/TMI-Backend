...
import LookupDA from "../DatabaseAccessLayer/lookup.da";
import { Note_Lookup_Type, Template_IDs } from "../Shared/constants";
import SchedulerDA from "../DatabaseAccessLayer/scheduler.da";
...
export default class ClientService {
  ...
  GetPatientInfo = async (req: Request, res: Response) => {
    try {
      ...
      const [[patient_info], last_appoinment, next_appoinment] =
        await this.clientDA.GetPatientInfo(client_Id as string, userid);
  ...
  SavePatintNote = async (req: Request, res: Response) => {
    try {
      ...
      const lookup_rows = await this.lookupDA.ValidateLookup(
        Note_Lookup_Type,
        resourceType
      );
      if (lookup_rows.length > 0) {
        if (!noteId) {
          notes_rows = await this.clientDA.ValidateNoteForMeeting(
            clientId,
            meetingId,
            resourceType
          );
        ...
        if (err_msg.length == 0) {
          ...
          if (client_rows.length > 0) {
            const meeting_rows = await this.clientDA.ValidateMeetingId(
              meetingId
            );
              ...
              if (resourceType == "PN") {
                ...
                try {
                  const result = await axios.post(
                    (process.env.EMAIL_ENGINE_URL as string) +
                      "/users/sendmail",
                    {
                      templateId: Template_IDs.Note,
                      email: clientIden,
                      data: resources,
                    }
                  );
      ...
      } else err_msg.push(ErrorMessage.InvalidLookupValue);
    ...
    try {
      ...
      if (!file) {
        return res.status(400).send({
          ...
          msg: ErrorMessage.FileNotAttached,
        });
      }
      ...
      if (client_rows.length > 0) {
        ...
        return res
          .status(200)
          .send({ status: SuccessMessage.Success, Location });
      }
      return res.status(400).send({
        status: ErrorMessage.Failed,
      ...
      if (notes.length > 0) {
        notes.map((note, index) => {
          notes[index].Resource =
            notes[index].Resource_Type == "PN"
              ? JSON.parse(notes[index].Resource).Notes
              : notes[index].Resource;
        });
