...
import {
  ...
  EncryptData,
  FormatDate,
  GetCalendarMaxdate,../Shared/constants
  IsEmail,
} from "../lib/commonFunctions";
import MemberDA from "../DatabaseAccessLayer/member.da";
...
import UserDA from "../DatabaseAccessLayer/user.da";
import shortid from "shortid";
import axios from "axios";
import { ErrorMessage, SuccessMessage } from "../VO/messages";
import ClientDA from "../DatabaseAccessLayer/client.da";
...
export default class MemberService {
  ...
  GetUserCalender = async (req: Request, res: Response) => {
    try {
      ...
      const [Access_Token, Refresh_Token] = await this.GetDecryptedToken(
        userid
      );
      ...
      if (Access_Token.length > 0) {
        const events = await GoogleAPIClient.ListCalenderEvent(
          Access_Token,
          Refresh_Token,
          new Date().toISOString(),
          GetCalendarMaxdate().toISOString()
        );

        if (events) {
          await this.memberDA.SaveMeetingDetailsFromGoogleCalendar(
            events,
            userid
          );
        }
        let [meeting_details, client_details] = await this.memberDA.GetMeetings(
          userid
        );
        let event_info = this.FormatEventDetails(
          meeting_details,
          client_details
        );
  ...
  CreateAppoiment = async (req: Request, res: Response) => {
    try {
      ...
      const event = (await GoogleAPIClient.CreateCalenderEvent(
        Access_Token,
        Refresh_Token,
        eventDetails
      )) as any;
      ...
      await this.memberDA.CreateMeetingDetails(
        id,
        uri,
        name,
        sdateTime,
  ...
  CreateCalenderEventObject = (body: any) => {
    const { summary, sessionInfo, startTime, endTime, date, patientDetails } =
      body;
  ...
  GetUserOauth2Data = async (req: Request, res: Response) => {
    try {
      ...
      if (code) {
        ...
        } = await this.googleAPI.getToken(code as string);
        const ticket = await this.googleAPI.verifyIdToken({
          idToken: id_token as string,
        });
        const tokenInfo = await GoogleAPIClient.GetTokenInfo(
        ...
        if (signed_email) {
          if (user_email !== signed_email) {
            return res
              .status(200)
              .send([{ message: "Please use correct email" }]);
          }
          const date = new Date();
          date.setMonth(new Date().getMonth() - 1);
          await this.memberDA.CreateTokenDetails(
            user_id,
            signed_email,
            EncryptData(access_token, user_id),
            EncryptData(refresh_token, user_id)
          );
          const events = await GoogleAPIClient.ListCalenderEvent(
            <string>access_token,
  ...
  CreateAuthURL = async (req: Request, res: Response) => {
    try {
      ...
      if (IsEmail(email)) {
        ...
        const url = await GoogleAPIClient.GenerateAuthURL(
          Oauth_Scopes,
          email as string
        );
  ...
  GetMemberInfo = async (req: Request, res: Response) => {
    try {
      ...
      const [IsOauthVerified, accessToken, refreshToken] = await this.HasOauth(
        user_id
      );

      let appoinments = await this.GetNextAndUpcomingAppoinment(
        accessToken,
        refreshToken,
        IsOauthVerified
      );
      let [
        ...
        [{ Total_Patient }],
      ] = await this.memberDA.GetMemberInfo(user_id);
  ...
  GetNextAndUpcomingAppoinment = (
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (IsOauthVerified) {
        try {
          let events = await GoogleAPIClient.ListCalenderEvent(
            access_token as string,
            ...
            for (let j = 0; j < events.length; j++) {
              ...
              if (attendees) {
                for (let i = 0; i < attendees.length; i++) {
                  ...
                  if (!self) {
                    let Name = null;
                    const client_rows = await this.clientDA.FindClientDetails(
                      email
                    );
              ...
              } else appoinments.push(appoinmentObj);
            }
  ...
  GetHolidayListAndSave = async (req: Request, res: Response) => {
    try {
      const holidayList = await axios.get(
        `https://www.googleapis.com/calendar/v3/calendars/en.indian%23holiday%40group.v.calendar.google.com/events?key=${process.env.GOOGLE_API_KEY}`
      );
  ...
  GetMemberProfile = async (req: Request, res: Response) => {
    try {
      ...
      const [[member_info], [{ Total_Patient }]] =
        await this.memberDA.GetMemberProfiledata(userid);
  ...
  UpdateAppoinment = async (req: Request, res: Response) => {
    try {
      ...
      const meeting_rows = await this.memberDA.ValidateMeetingIdWithUserId(
        meetingId,
        userid
      );
      if (meeting_rows.length > 0) {
        ...
        const event = (await GoogleAPIClient.UpdateCalendarEvent(
          Access_Token,
          Refresh_Token,
          Event_Id,
          eventDetails
        ...
        try {
          ...
          return res
            .status(200)
            .send({
              msg: SuccessMessage.MeetingUpdated,
        ...
        return res
          .status(400)
          .send({
            msg: ErrorMessage.MeetingNotExist,
            status: ErrorMessage.Failed,
          });
      }
    } catch (error) {
  ...
  GetDecryptedToken = async (userid: string) => {
    try {
      ...
      if (token_rows.length > 0) {
        ...
        return [
          DecryptData(Access_Token, userid),
          DecryptData(Refresh_Token, userid),
        ];
      }
      return ["", ""];
    } catch (error) {
