...
import { Template_IDs } from "../Shared/constants";
...
export default class UserService {
  ...
  Signup = async (req: Request, res: Response) => {
    try {
      ...
      const user_details_rows = await this.userDA.CheckIfUserExists(
        phone_number
      );
      ...
      if (user_details_rows.length > 0) {
        ...
        if (["N", "Y"].includes(Is_Active)) {
          ...
          else message = ErrorMessage.SamePh;
          ...
          await this.userDA.UpdatePracDetails(
            major,
            spec,
            experience,
            firebase_id
          ...
          try {
            const result = await axios.post(
              (process.env.EMAIL_ENGINE_URL as string) + "/users/sendmail",
              { templateId: Template_IDs.Signup, email: Email }
            );
  ...
  RefreshToken = async (req: Request, res: Response) => {
    try {
      ...
      const refresh_token_rows = await this.userDA.GetRefreshToken(
        refreshtoken
      );
      if (refresh_token_rows.length > 0) {
        ...
        if (refresh_token != refreshtoken) {
          ...
          await this.userDA.UpdateRefreshToken(
            refresh_token,
    ...
    try {
      ...
      if (user_rows.length > 0) {
        ...
        if (user_status == "I")
          return res
            .status(200)
            .send([{ message: "Please complete your registration" }]);
        let firebase_id = await this.GetFirebaseIdByPhoneNumber(phone_number);
  ...
  private GetFirebaseIdByPhoneNumber = async (phone_number: string) => {
    try {
      ...
      const userRecord: admin.auth.UserRecord = await admin
        .auth()
  ...
  RegisterUsingReferalCode = async (req: Request, res: Response) => {
    try {
      ...
      if (msg == 1) return res.status(200).send();
  ...
  UpdateMemberDetails = async (req: Request, res: Response) => {
    try {
      ...
      if (file) {
        memberDetails.Resume_URL = await (
          await AwsClient.UploadFileToS3(file)
        ).Location;
      }
      console.log(userid);
      const [[[result]]] = await this.userDA.UpdateMemberDetails(
        memberDetails,
        userid
      );
      console.log(result);
