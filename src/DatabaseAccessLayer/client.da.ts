...
export default class ClientDA extends DbConnection {
  ...
  async GetPatientList(memberId?: string) {
    try {
      return this.ReadDB<any[]>(DBsp.GetPatientListBymemberId, [
        memberId ?? null,
      ]);
    } catch (error) {
      throw error;
  ...
  async ValidateNoteId(noteId: string, resourceType: string) {
    return this.ReadDB<any[]>(DBqueries.CheckIfNotesExists, [
      noteId,
      resourceType,
    ]);
  }

  async ValidateNoteForMeeting(
    clientId: string,
    meetingId: string,
    resourceType: string
  ) {
    return this.ReadDB<any[]>(DBqueries.CheckIfNotesForMeetingExists, [
      clientId,
      meetingId,
      resourceType,
    ]);
  ...
  async SaveFile(
    ...
    await this.InsertOrUpdateDB(
      [DBqueries.SaveFile],
      [[GenerateUUID(), clientId, memberId, meetingId, filePath]]
    );
  }
