...
export default class MemberDA extends DbConnection {
  ...
  async CreateMeetingDetails(
    ...
    try {
      ...
      paramsList.push([
        attendeeListFromGoogle.replace(/(^,)|(,$)/g, ""),
        "CLIENT",
        eventId,
      ]);
  ...
  async SaveMeetingDetailsFromGoogleCalendar(
  ...
  async GetMeetings(user_id: string, meetingId?: string) {
    try {
      return this.ReadDB<[IMeetingDetails[], IClientDetails[]]>(
        DBsp.GetUpcomingMeetings,
        [user_id, meetingId ?? null]
      );
    } catch (error) {
      throw error;
    }
  ...
  async ValidateMeetingIdWithUserId(meetingId: string, userid: string) {
    try {
      return this.ReadDB<any[]>(DBqueries.ValidateMeetingIdWithUserId, [
        meetingId,
        userid,
        "Y",
      ]);
