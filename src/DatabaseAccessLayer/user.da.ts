...
export default class UserDA extends DbConnection {
  ...
  async CheckIfUserExists(phone_number: any): Promise<IPhoneNumber[]> {
    try {
      return await this.ReadDB<IPhoneNumber[]>(DBqueries.CheckIfUserExists, [
        phone_number,
      ]);
    } catch (error) {
      throw error;
  ...
  async CheckIfUserExistsByUserId(user_id: string): Promise<IPhoneNumber[]> {
    try {
      return await this.ReadDB<IPhoneNumber[]>(
        DBqueries.CheckIfUserExistsByUserId,
        [user_id]
      );
    } catch (error) {
      throw error;
  ...
  async InsertUserDetails(
    ...
    try {
      ...
      queryList.push(
        user_type == "P"
          ? DBqueries.InsertPracDetails
          : DBqueries.InsertSeekerDetails
      );
  ...
  async UpdateUserDetails(
    ...
    try {
      await this.InsertOrUpdateDB(
        [DBqueries.UpdateUserDetails],
        [[frst_name, last_name, email, refresh_token, phone_number, ActiveFlag]]
      );
    } catch (error) {
      throw error;
    }
  ...
  async GetRefreshToken(refresh_token: string): Promise<IRefreshToken[]> {
    try {
      return await this.ReadDB<IRefreshToken[]>(DBqueries.CheckRefreshToken, [
        refresh_token,
      ]);
    } catch (error) {
      throw error;
  ...
  async UpdateRefreshToken(
    ...
    try {
      await this.InsertOrUpdateDB(
        [DBqueries.UpdateRefreshToken],
        [[refresh_token, firebase_id]]
      );
    } catch (error) {
      throw error;
    }
  ...
  async UpdateMemberDetails(memberDetails: IMember, userid: string) {
    try {
      ...
      return await this.InsertOrUpdateDB(
        [DBsp.UpdateMember],
        [
          [
            frst_name,
  ...
  async GetUserIdByExternalUserId(exId: any) {
    try {
      return await this.ReadDB<any[]>(DBqueries.GetUserIdByExternalId, [
        exId,
        "Y",
      ]);
    } catch (error) {
  ...
  async CheckIfEmailAlreadyExists(emailId: string): Promise<IPhoneNumber[]> {
    try {
      return await this.ReadDB<IPhoneNumber[]>(
        DBqueries.CheckIfEmailAlreadyExists,
        [emailId]
      );
    } catch (error) {
      throw error;
