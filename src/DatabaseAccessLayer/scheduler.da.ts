...
export default class SchedulerDA extends DbConnection {
  ...
  async CreateOrUpdateJob(jobDetails: IJobDetais) {
    try {
      const { Job_Id, Job_Name, Start_TS, Run_Status, Scheduled_TS, IsUpdate } =
        jobDetails;
      if (!IsUpdate)
        await this.InsertOrUpdateDB(
          [DBqueries.CreateJob],
          [[Job_Name, Job_Id, Start_TS, Run_Status, Scheduled_TS]]
        );
      else if (IsUpdate) {
        await this.InsertOrUpdateDB(
          [DBqueries.UpdateJob],
  ...
  async CreateApiLog(logDetails: ILog) {
    try {
      ...
      await this.InsertOrUpdateDB(
        [DBqueries.CreateApiLogs],
        [[Process_Name, Key1, Value1, Response]]
      );
    } catch (error) {
      throw error;
    }
